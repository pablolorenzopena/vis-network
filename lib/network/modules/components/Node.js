...
import CustomImageShape from "./nodes/shapes/CustomImageShape";
import EnhancedImageShape from "./nodes/shapes/EnhancedImageShape";
...
class Node {
    ...
    if (currentShape === this.options.shape && this.shape) {
      ...
      switch (this.options.shape) {
        ...
        case "triangleDown":
          ...
          break;
        case 'customImage':
          this.shape = new CustomImageShape(this.options, this.body, this.labelModule, this.imageObj);
          break;
        case 'enhancedImage':
          this.shape = new EnhancedImageShape(this.options, this.body, this.labelModule, this.imageObj);
          break;
        default:
          this.shape = new Ellipse(this.options, this.body, this.labelModule);
          break;
   ...
   *
   * @returns {string|undefined} title    The title of the node, or undefined when no title
   *                           has been set.
   */
  getTitle(pointerFromInteraction) {
    console.log("llamandose con:", this.id, pointerFromInteraction);
  if (
    this.shape?.iconHitboxes?.length &&
    pointerFromInteraction &&
    typeof pointerFromInteraction.x === "number"
  ) {
    const { x, y } = pointerFromInteraction;

    for (const hitbox of this.shape.iconHitboxes) {
      const insideX = x >= hitbox.x && x <= hitbox.x + hitbox.width;
      const insideY = y >= hitbox.y && y <= hitbox.y + hitbox.height;
      if (insideX && insideY) {
        console.log("🟢 Found icon hitbox for node", this.id, "at", hitbox);
        return hitbox.icon.tooltip;
      }
    }
  }
  console.log("🔴 No icon hitbox found for node", this.id);
  return this.options.title;
}

handleIconClick(x, y) {
  console.log("Handling icon click at:", x, y);
  if (!this.shape?.iconHitboxes?.length) return false;

  for (const hitbox of this.shape.iconHitboxes) {
    const insideX = x >= hitbox.x && x <= hitbox.x + hitbox.width;
    const insideY = y >= hitbox.y && y <= hitbox.y + hitbox.height;
    if (insideX && insideY && typeof hitbox.icon?.onClick === "function") {
      hitbox.icon.onClick();
      return true;
    }
  }
  ...
  if (this.labelModule.visible()) {
    if (pointInRect(this.labelModule.getSize(), point)) {
      console.log("🟡 Hovered label of node", this.id);
      ret.push({ nodeId: this.id, labelId: 0 });
    }
  }

  if (pointInRect(this.shape.boundingBox, point)) {
    console.log("🔵 Hovered bounding box of node", this.id);
    ret.push({ nodeId: this.id });
  }

  if (this.shape instanceof EnhancedImageShape) {
    const hoveredIcon = this.shape.getIconAt?.(point.x, point.y);
    if (hoveredIcon && hoveredIcon.tooltip) {
      console.log("🟢 Hovered icon with tooltip:", hoveredIcon.tooltip);
      ret.push({
        nodeId: this.id,
        labelId: "__customIcon__",
        tooltip: hoveredIcon.tooltip,
      });
    }
  }
