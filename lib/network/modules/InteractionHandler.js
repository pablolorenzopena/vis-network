...
class InteractionHandler {
  ...
  if (node && typeof node.handleIconClick === "function") {
    const canvasX = this.canvas._XconvertDOMtoCanvas(pointer.x);
    const canvasY = this.canvas._YconvertDOMtoCanvas(pointer.y);

    const clicked = node.handleIconClick(canvasX, canvasY);
    if (clicked) {
      return; // 🔥 Se ha hecho clic en un icono: NO continuar con selección
    }
  }
  ...
  _checkShowPopup(pointer) {
    ...
    if (this.popupObj === undefined) {
      ...
      for (let i = 0; i < nodeIndices.length; i++) {
        ...
        if (node.isOverlappingWith(pointerObj) === true) {
          ...
          if (node.getTitle({x,y}) !== undefined) {
    ...
    if (this.popupObj !== undefined) {
      ...
      if (this.popupObj.id !== previousPopupObjId) {
        ...
        // popup area
        this.popup.setPosition(pointer.x + 3, pointer.y - 5);
        console.log("Popup title:", this.popupObj.getTitle({x, y}));
        console.log(this.popup);
        console.log(this.popup);
        this.popup.setText(this.popupObj.getTitle({x,y}));
        this.popup.show();
